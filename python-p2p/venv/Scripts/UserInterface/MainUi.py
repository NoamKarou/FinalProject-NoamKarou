...
from Scripts.UserInterface.Components import (
    UserAuthenticationField, UserCreation, ConnectToServer, MainMenu, UserDisplayWidget, LoginMenu, AccountCreation, MinerCheckbox)
...
class Ui:
    ...
    def ui_loop(self):
        ...
        target_width = int(3 / 5 * screen_width)
        target_height = int(3 / 5 * screen_height)

        self.root.geometry(f"{target_width}x{target_height}")

        b1 = ttk.Button(self.root, text="Button 1", bootstyle=INFO)
    ...
    def account_creation_ui(self):
        ...
        login_object = AccountCreation.AccountCreation(
            ...
            creation_callback=self.interface.create_account)
